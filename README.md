...
2026-09-23 补测。前两个参照（v1.6、nwrt）都是社区构建，不是这块板子的出厂固件；这次
测的是**真·出厂原厂固件**——OpenWrt Chaos Calmer 501.8 / SPF11.4_CSU2，内核 4.4.60、
**32 位**。原厂默认不开 SSH，为取基线用 U-Boot 网页恢复把我们的 initramfs 在内存里
跑起来、挂原厂 overlay 临时开了 SSH（未刷任何引导分区）。完整信息与整机备份见
`oem-baseline/`（含真实凭据，未入库）。

| iperf3 `-P 4` | 原厂 501.8 | 本构建 |
|---|---|---|
| 有线 下 / 上 | 947 / 947 Mbps，主机 CPU 9 % / 6 % | 949 / 949 |
| 5 GHz 下 / 上（160 MHz） | 612 / 899 Mbps，CPU 15–16 % | — |
| 2.4 GHz 下 / 上（HT20） | 114 / 65 Mbps，CPU 8 % | — |

（原厂 CPU 是传输中段 `/proc/stat` 的原始 busy%，未减空闲基线，与上表本构建那列的
增量口径不同，只读作「两核都远未跑满」；有线两边都顶客户端千兆线速。）
