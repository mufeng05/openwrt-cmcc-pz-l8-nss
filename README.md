...
原厂那一列有空格，是因为当时没有逐项测 nwrt。后来已经把 nwrt 刷回这块板子
实测过一遍——同一个客户端、同一个服务端、同一个下午——但用的是四路并发 HTTP
而不是 iperf3，所以那些数字单独列表，不填进上面的空格：

| 四路并发 HTTP，20 秒 | nwrt | 本构建 |
|---|---|---|
| 有线 LAN → WAN | 901.8 Mbit/s | 900.9 |
| **5 GHz LAN → WAN** | **722** | 425 |
| 2.4 GHz LAN → WAN | 119.4 | 尚未用此法测 |
| 5 GHz 时主机 CPU | 16.1 % | 尚未用此法测 |

5 GHz 的差距主要是信道宽度，不是什么谜团：nwrt 把 QCN6122 跑在 **160 MHz**，
客户端协商到 1922 Mbit/s；ath11k 跑 80 MHz，同一个客户端协商到 1201。代价是
nwrt 要花掉两个主机核的 16 %。其余在 [docs/WIFILI.md](docs/WIFILI.md)，包括配置
差异，以及本构建唯一一处有意偏离原厂的地方。
