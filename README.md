...
![状态概览页，截于转发 986 Mbit/s 时](docs/img/overview.png)

整页截于**转发 986 Mbit/s 时**。本构建新增的是其中的「硬件」一节：CPU 占用 14 %、
NSS 占用 14 %、14 条加速连接，两个口一进一出各跑满千兆。数据面在 NSS 核里，主 CPU
基本是闲着的——这一节存在的意义就是让这件事看得见。客户端的主机名、MAC、DUID 和
地址做了涂抹处理。
