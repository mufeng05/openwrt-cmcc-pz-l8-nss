...
- **厂商系社区固件（v1.6 / nwrt）** 快，但都建立在闭源的 qca-wifi 上，内核停在
  `4.4.60` 和 `5.4.250`，而且都是 **32 位**——两块都报 `armv7l`，尽管 IPQ5018
  是 ARMv8。也没有能用的源码树：拿到什么就是什么，改不了，也编不出来。
- **OpenWrt 官方镜像**内核新、是 64 位，但不支持 NSS。转发回到 Linux，DSA 路径上
  502 Mbit/s 就能把两个核跑满。
- 而且**官方 25.12.x 的镜像连 WiFi 都起不来**。
  `target/linux/qualcommax/image/ipq50xx.mk` 里的 `Device/cmcc_pz-l8` 没有定义
  `DEVICE_PACKAGES`（同一个文件里别的设备定义了 12 处），所以 ath11k 固件和这块
  板子的 board data 都不在镜像里，射频根本不会 probe。快照分支（r31246）是带的，
  本项目用 `feed/ipq-wifi-cmcc_pz-l8` 把它补回来。

所以这个构建要的是三者的交集：**官方基线、64 位新内核、能自己编译，再加上 NSS
硬件卸载**。

原生 OpenWrt 在这块板子上用 DSA 走 Linux 转发路径。加上 NSS 之后：

| iperf3 `-P 4`，每方向 15 秒 | 本构建 | 原厂 501.8 | nwrt（闭源栈） | 原生 OpenWrt |
|---|---|---|---|---|
| **有线 ↔ WAN** | **949 上 / 949 下 Mbps，+0 % CPU** | 947 / 947，9 % / 6 %※ | 924 / 926 @ 4 % | **502 @ 100 % CPU** |
| **5 GHz ↔ WAN** | 510 上 / 438 下 Mbps，+0.3 % CPU（80 MHz） | 612 / 899（160 MHz），15–16 %※ | ~624 @ 25–35 % | 308 |
| **2.4 GHz ↔ WAN** | 58 上 / 61 下 Mbps，+0.3 % CPU | 114 / 65，8 %※ | — | — |

CPU 指两个 Cortex-A53 主机核（NSS 的 UBI32 核独立，不计），本构建那列是**增量值**——
全程每秒采样 `/proc/stat`，减去同一次采集的空载基线，因为采样器自己就要吃
2–5 %。有线那一行落在基线上或以下：949 Mbps 的开销已经小到这个方法分辨不出来。

> ※ **原厂那列**是 2026-09-23 用同样的 iperf3 `-P 4` 补测的（这台设备的真·出厂固件，
> 详见下文「原厂固件基线」）。它的 CPU 是传输中段 `/proc/stat` 的**原始 busy%**（未减
> 空闲基线），与本构建列的增量口径不同，只读作「两核远未跑满」。原厂 5 GHz 是 160 MHz、
> 本构建这列是 80 MHz 的早期数——同宽度的对照见下面「三方同法对比」。

「原生 OpenWrt」那一列的 5 GHz 数字必然不是官方镜像的开箱状态——没有固件和 board
data，射频不会 probe，那一格只能是把缺的两个包补齐之后才测得出来的。
...
| 四路并发 HTTP，20 秒 | 原厂 501.8 | v1.6 | nwrt | 本构建 |
|---|---|---|---|---|
| 有线 LAN → WAN | 902.9 | 912.1 | 901.8 | **912.6** |
| **5 GHz @ 160 MHz** | 711 | **730** | **722** | **697** |
| 5 GHz @ 80 MHz | — | — | — | 462 |
| 2.4 GHz @ 20 MHz | ~92（59–124）※ | 98.8 | **119.4** | 75.0 |
| 2.4 GHz @ 40 MHz（`noscan`）| — | — | — | **112.5** |
| 5 GHz 传输时主机 CPU | 17 % @ 711 | 测不出，见下 | 16.1 % @ 537 | **2.9 % @ 441** |

现在**「原厂」这一列就是这台设备未改动的出厂固件**（501.8，2026-09-23 同法补测，
见下文「原厂固件基线」）。另外两个参照仍不是出厂固件：**v1.6** 是厂商 4.4 SDK 上的
社区构建，**nwrt** 是 5.4 上的社区构建，两者都用闭源 qca-wifi。

> ※ 原厂 2.4 GHz 波动很大：六轮 51.8 / 59.1 / 69.7 / 113.9 / 116 / 124，随邻区拥塞
> 在 ~59 与 ~120 两态间跳（这台在真实环境里，周围二十多个网络）。好态 ~116 与 iperf3
> 表的 114 吻合。
...
2026-09-23 补测。这台设备的**真·出厂原厂固件**——OpenWrt Chaos Calmer 501.8 /
SPF11.4_CSU2，内核 4.4.60、**32 位**。原厂默认不开 SSH，为取基线用 U-Boot 网页恢复
把我们的 initramfs 在内存里跑起来、挂原厂 overlay 临时开了 SSH（未刷任何引导分区）。
**吞吐已并入上面两张表的「原厂」列**（iperf3 与四路 HTTP 各测一份）；完整信息与整机
备份见 `oem-baseline/`（含真实凭据，未入库）。

几条只有拿到真原厂才能坐实的事：

- **原厂 NSS 固件是 `NSS.MP.11.4-10-R`**（md5 `f4f67348…`），不是 12.5。这补上了
  此前「原厂到底用哪版」的空缺：nwrt（第三方）用 12.5，v1.6（社区）用 11.4-3-R，
  **出厂固件是 11.4-10-R**，三者互不相同。本构建改用 12.5 的依据（自测 +65%）不受影响。
- **原厂同时加载 ECM 和 SFE（`shortcut_fe`），但 SFE 不干活**：ECM 只编了 NSS 前端
  （`ecm_nss_ipv4`，没有 `ecm_sfe`）；打流时 6 条连接全落在
  `ecm_nss_ipv4/accelerated_count`，SFE 连接表为空——被 NSS 硬件加速的流不过 Linux
  转发路径，SFE 的钩子看不到。原厂那 947 / 612 / 899 全是 NSS 的功劳。本构建同样
  只用 ECM→NSS（此平台缺 SFE 需要的三个内核钩子）。
- **内存切分与 nwrt 同档、与本构建不同**：原厂只把 ~174 MB 交给内核、预留 11 MB
  （`Memory: 167140K/178176K … 11036K reserved`）；本构建交足 256 MB、预留 86 MB。
- **NSS 配置 `n2h pool 4096 + high_water 16336`** 在真原厂上确认（此前只在 nwrt 上见过）。
