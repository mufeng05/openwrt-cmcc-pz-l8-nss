...
| 2.4 GHz @ 40 MHz | — | — | **112.5** |
| 5 GHz 时主机 CPU | 测不出，见下 | 16.1 % @ 537 | **2.9 % @ 441** |

**同样 160 MHz 下，本构建与两个参照相差 3.5–4.6%**。此前把 5 GHz 差距归因于
信道宽度是推断（722/425 = 1.70，带宽比 2.0），现在是实测：把本构建也开到
160 MHz，461.6 → 696.9，涨 51%，剩下的差距小到不需要另找原因。

CPU 那一格现在才是同等条件比较（同频段、同宽度、同测法，各自带空闲对照）：
nwrt 花 5.6 倍的 CPU 只多搬 22% 的数据。

160 MHz 需要设国家码（默认 `country 00` 下 5 GHz 被切成两个 80 MHz 规则块，
根本不存在 160 MHz 信道），且该信道覆盖 DFS 频段，每次启动要先做 62 秒 CAC
才开始发信标。所以这不是镜像默认值——监管域本就不该由镜像替用户决定。

本构建这一列是刷回设备后重测的，`n2h_high_water_core0` 已改回原厂的 16336。
主机 CPU 那一格带空闲对照：无流量时 3.1 %，传输中 5.9 %，边际 +2.8 个百分点。
折算到每 Mbit/s 是 0.017 %，nwrt 是 0.030 %——但 nwrt 那些比特走的是 160 MHz，
我们走 80 MHz，不是同等条件的比较。
...
`XXXX` 是本机 MAC 的后两字节（大写，无分隔），首次启动时由
`/etc/uci-defaults/97-pzl8-wifi-ssid` 补上——和原厂固件同一套命名方式，
这样两块板子跑同一个镜像不会撞名。改成别的名字随意：那个脚本只会动它还
认得出是出厂默认的名字。
